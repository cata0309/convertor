...
//  sf::Vector2f back_box_size = sf::Vector2f(input_form.max_chars_view*14, 40);
...
void handleKeyEvent(InputForm &input_form, char c) {
  ...
  if (int(c)==8) {
    ...
    aux[0] = c;
    aux[1] = '\0';
    input_form.input.insert(input_form.input.getSize() + input_form.insert_position, aux);
    input_form.ruler++;
...
void updateViewedString(InputForm &input_form) {
  ...
  input_form.with_carret.insert(input_form.input.getSize() + input_form.insert_position, "|");
  input_form.without_carret = input_form.input;
  input_form.without_carret.insert(input_form.input.getSize() + input_form.insert_position, " ");
  if (input_form.focus) {
    if (!input_form.carret) {
      if (input_form.without_carret.getSize() >= input_form.max_chars_view)
        input_form.viewed_string =
            input_form.without_carret.substring(input_form.start_index, input_form.max_chars_view);
      else
        input_form.viewed_string = input_form.without_carret;
    } else {
      if (input_form.with_carret.getSize() >= input_form.max_chars_view)
        input_form.viewed_string = input_form.with_carret.substring(input_form.start_index, input_form.max_chars_view);
      else
